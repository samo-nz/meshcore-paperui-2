...
public:
  ...
  // PaperUI extensions. These remain in the companion serializer so they are
  // forward-compatible and do not alter MeshCore's wire protocol.
  uint8_t fast_gps_channel_idx = 0xFF; // disabled, or a non-public channel index
  uint8_t fast_gps_region = 0;         // 0 = unscoped; other values select a region
...
private:
  ...
  protected:
    void structure() override {
      ...
      def("fast_gps_ch", _parent->fast_gps_channel_idx);
      def("fast_gps_reg", _parent->fast_gps_region);
