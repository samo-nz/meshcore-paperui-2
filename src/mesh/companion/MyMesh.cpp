...
// Mirror a successful send made by the connected companion app into PaperUI.
// The queue is the thread-safe boundary: MyMesh must never modify LVGL/model
// state directly from the protocol loop.
static void pushLocalUiMessage(const char* sender, const char* text, uint8_t channel_idx = 0xFF) {
  mesh::bridge::MessageIn message = {};
  if (sender) StrHelper::strzcpy(message.sender_name, sender, sizeof(message.sender_name));
  if (text) StrHelper::strzcpy(message.text, text, sizeof(message.text));
  message.timestamp = 0;
  message.is_direct = channel_idx == 0xFF;
  message.is_channel = channel_idx != 0xFF;
  message.channel_idx = channel_idx;
  message.is_self = true;
  mesh::bridge::push_message(message);
}
...
uint8_t MyMesh::onContactRequest(const ContactInfo &contact, uint32_t sender_timestamp, const uint8_t *data,
  ...
  if (data[0] == REQ_TYPE_GET_TELEMETRY_DATA) {
    ...
    if (permissions & TELEM_PERM_BASE) { // only respond if base permission bit is set
      ...
      float temperature = mc_board.getMCUTemperature();
...
// DISPLAY_CLASS guard removed for BridgeUITask
  ...
  mc_board.setLoRaFemLnaEnabled(_prefs.radio_fem_rxgain);
  mc_board.setLoRaFemPaGainEnabled(_prefs.radio_fem_txgain);
  ...
  if (cmd_frame[0] == CMD_DEVICE_QUERY && len >= 2) { // sent when app establishes connection
    ...
    if (recipient && (txt_type == TXT_TYPE_PLAIN || txt_type == TXT_TYPE_CLI_DATA)) {
      ...
      if (result == MSG_SEND_FAILED) {
        ...
        if (txt_type == TXT_TYPE_PLAIN) {
          pushLocalUiMessage(_prefs.node_name, text);
        }
      }
    } else {
    ...
    if (txt_type != TXT_TYPE_PLAIN) {
      ...
      if (success && sendGroupMessage(msg_timestamp, channel.channel, _prefs.node_name, text, len - i)) {
        writeOKFrame();
        pushLocalUiMessage(_prefs.node_name, text, channel_idx);
      } else {
...
// DISPLAY_CLASS guard removed for BridgeUITask
    ...
    if (gps_supported) {
      ...
      // Region scope for fast-GPS beacons. The region names remain an internal
      // firmware/UI detail: CMD_GET_CUSTOM_VARS uses commas between name:value
      // records, so publishing a comma-separated value here corrupts the frame
      // and makes the official app fail while parsing all device variables.
