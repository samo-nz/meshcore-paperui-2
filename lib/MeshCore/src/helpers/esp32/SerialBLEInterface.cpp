...
void SerialBLEInterface::begin(const char* prefix, char* name, uint32_t pin_code) {
  ...
  // The device displays a fixed passkey for entry on the phone.
  ...
  // Encryption is enforced here; SC_MITM_BOND remains enforced by the link.
  // Attribute-level MITM rejects the app's first write during Android pairing.
  pTxCharacteristic->setAccessPermissions(ESP_GATT_PERM_READ_ENCRYPTED);
  pTxCharacteristic->setCallbacks(this); // report notification delivery failures
...
void SerialBLEInterface::onStatus(BLECharacteristic*, Status status, uint32_t code) {
  notify_ok = (status == SUCCESS_NOTIFY);
  if (!notify_ok) BLE_DEBUG_PRINTLN("notify failed status=%d code=%u", (int)status, code);
}
...
void SerialBLEInterface::disable() {
  if (!_isEnabled || pServer == NULL || pService == NULL) return;
  _isEnabled = false;

  BLE_DEBUG_PRINTLN("SerialBLEInterface::disable");

  pServer->getAdvertising()->stop();
  // Avoid disconnecting the default/stale connection ID when advertising was
  // disabled before any client completed authentication.
  if (deviceConnected) {
...
size_t SerialBLEInterface::checkRecvFrame(uint8_t dest[]) {
  if (send_queue_len > 0   // first, check send queue
    ...
    // The app can write its first query before its CCCD enables notifications.
    // Arduino BLE silently drops notify() in that state, so keep the reply
    // until the client subscribes instead of logging a misleading success.
    BLE2902* ccc = (BLE2902*)pTxCharacteristic->getDescriptorByUUID(BLEUUID((uint16_t)0x2902));
    if (!deviceConnected || !ccc || !ccc->getNotifications()) {
      if (!ccc_wait_logged) {
        BLE_DEBUG_PRINTLN("waiting for notifications (CCC=%d, connected=%d)",
                          ccc ? ccc->getNotifications() : -1, deviceConnected);
        ccc_wait_logged = true;
      }
    } else {
      if (ccc_wait_logged) BLE_DEBUG_PRINTLN("notifications enabled; sending queued reply");
      ccc_wait_logged = false;
      _last_write = millis();
      notify_ok = false;
