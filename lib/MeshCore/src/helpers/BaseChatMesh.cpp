...
void BaseChatMesh::onAdvertRecv(mesh::Packet* packet, const mesh::Identity& id, uint32_t timestamp, const uint8_t* app_data, size_t app_data_len) {
  ...
  // Keep the authenticated advert for manual addition from the discovery UI.
  // Previously this was only saved after auto-add, so manually discovered
  // nodes appeared in Discover but could not be added without another advert.
