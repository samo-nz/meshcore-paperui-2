...
// The firmware sets this before epd_init(); panel prep must stay ahead of
// the feed deadlines or the user sees line-buffer underruns/blank screens.
int epd_prep_task_priority = 7;
...
static void IRAM_ATTR render_thread(void* arg) {
    ...
    while (true) {
        ...
        // Frequent fast e-paper updates otherwise keep both pinned prep
        // tasks runnable continuously and prevent IDLE0/IDLE1 from feeding
        // the watchdog. One tick between completed frames is sufficient.
        vTaskDelay(1);
    }
}
...
void epd_renderer_init(enum EpdInitOptions options) {
    ...
    for (int i = 0; i < NUM_RENDER_THREADS; i++) {
        ...
        RTOS_ERROR_CHECK(xTaskCreatePinnedToCore(
            // Below GT911 (6) and LVGL (5); BLE controller tasks retain their
            // higher priorities while panel prep continues on both cores.
            render_thread, "epd_prep", 1 << 12, (void*)i, epd_prep_task_priority,
