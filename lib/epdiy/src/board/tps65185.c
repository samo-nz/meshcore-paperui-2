...
void tps_set_vcom(i2c_port_t i2c_num, unsigned vcom_mV) {
    ...
    esp_err_t high_err = ESP_FAIL;
    esp_err_t low_err = ESP_FAIL;
    for (int attempt = 0; attempt < 3 && high_err != ESP_OK; ++attempt) {
        high_err = tps_write_register(i2c_num, 4, (val & 0x100) >> 8);
        if (high_err != ESP_OK) vTaskDelay(pdMS_TO_TICKS(2));
    }
    for (int attempt = 0; attempt < 3 && low_err != ESP_OK; ++attempt) {
        low_err = tps_write_register(i2c_num, 3, val & 0xFF);
        if (low_err != ESP_OK) vTaskDelay(pdMS_TO_TICKS(2));
    }
    if (high_err != ESP_OK || low_err != ESP_OK) {
        ESP_LOGE("epdiy", "VCOM write failed after retries");
    }
}
