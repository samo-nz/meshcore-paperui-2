...
static void epd_board_poweron(epd_ctrl_state_t *state) {
  ...
  // The TPS65185 shares I2C with touch, RTC and battery devices. A transient
  // NACK must not reboot the entire radio; retry briefly and skip this refresh
  // if the controller still cannot be reached.
  esp_err_t enable_err = ESP_FAIL;
  for (int attempt = 0; attempt < 3 && enable_err != ESP_OK; ++attempt) {
    enable_err = tps_write_register(config_reg.port, TPS_REG_ENABLE, 0x3F);
    if (enable_err != ESP_OK) vTaskDelay(pdMS_TO_TICKS(2));
  }
  if (enable_err != ESP_OK) {
    ESP_LOGE("epdiy", "TPS65185 enable failed after retries: %s", esp_err_to_name(enable_err));
    return;
  }
